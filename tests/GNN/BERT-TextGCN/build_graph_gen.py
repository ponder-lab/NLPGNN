...
try:
    ...
    # The 2018 GCS URL serves a 720-byte AccessDenied body, and the loader's size
    # check compares against that Content-Length, so the staged HF-mirror checkpoint
    # can never validate. The extracted dir is already in place; skip the unzip.
    LoadCheckpoint.unzip = lambda self, f: None
    load_check = LoadCheckpoint(language='en', cased=False)
...
def build_trunk(model, input_file, label2index, node2index, num, mode="train", drw_attention=False):
    nodes = []
    labels_all = []
    edge_lists = []
    edge_weights = []
